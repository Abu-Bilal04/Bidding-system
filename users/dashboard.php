          ...
          <div class="row mt-4">
            ...
            $query = "
                SELECT 
                    ...
                    -- Get the highest bid for each product, fallback to product price
                    COALESCE(MAX(b.bid_price), p.price) AS display_price
                FROM products p
            ...
            if (mysqli_num_rows($products) > 0) {
                while ($row = mysqli_fetch_assoc($products)) {
                    ...
                    <div class="col-lg-3 d-flex grid-margin stretch-card">
                      <div class="card">
                        <img src="../backend/products/' . htmlspecialchars($row['image']) . '" class="card-img-top p-3" alt="Item Image">
                        <div class="card-body">
                          <h5 class="card-title">' . htmlspecialchars($row['product']) . '</h5>
                          <p class="card-text mb-1"><strong>Category:</strong> ' . htmlspecialchars($row['category_name']) . '</p>
                          <p class="card-text mb-1"><strong>Price:</strong> $' . number_format($row['display_price'], 2) . '</p>
                          
                          <!-- Countdown -->
                          <p class="card-text mb-3">
                            <strong>Time Left:</strong> 
                            <span class="countdown" data-deadline="' . $row['deadline'] . '"></span>
                          </p>

                          <a href="place_bid.php?id=' . $row['id'] . '" class="btn btn-primary btn-sm w-100">Place Bid</a>
                        </div>
                      </div>
                    </div>';
                }
                ...
                echo '<div class="col-12 text-center">
                  <img src="../images/no-data.webp" width="300" />
                  <h5 class="text-muted mt-5">No products available at the moment.</h5>
                </div>';
            }
            ?>
