      ...
      <nav class="bottom-navbar">
        <div class="container">
          <ul class="nav page-navigation">
            ...
            <li class="nav-item active"><a href="bidding.php" class="nav-link active"><i class="mdi mdi-gavel menu-icon"></i><span class="menu-title">Bidding</span></a></li>
    ...
    <div class="container mt-5">
      <div class="row">
        <div class="col-md-12">
          <div class="card">
            <div class="card-body">
              <h4 class="card-title mb-4 text-primary">Auction List</h4>
...
$bids = mysqli_query($dbcon, "
    SELECT b.id, b.product_id, b.product_name, b.current_price, b.fullname, b.email, b.phone, b.bid_price, p.deadline
    FROM bids b
    INNER JOIN (
        SELECT product_id, MAX(bid_price) AS max_bid
        FROM bids
        GROUP BY product_id
    ) AS highest
    ON b.product_id = highest.product_id AND b.bid_price = highest.max_bid
    INNER JOIN products p ON b.product_id = p.id
    ORDER BY b.id DESC
");

while ($row = mysqli_fetch_assoc($bids)) {
    echo "<tr class='bid-row'>
            <td>{$sn}</td>
            <td>{$row['product_name']}</td>
            <td>" . number_format($row['current_price']) . "</td>
            <td>" . number_format($row['bid_price']) . "</td>
            <td>{$row['fullname']}</td>
            <td>{$row['email']}</td>
  ...
  timers.forEach(timer => {
    ...
    const interval = setInterval(() => {
      ...
      if (distance <= 0) {
        ...
        timer.innerHTML = "<b class='text-success'>CLOSED</b>";
        return;
      }
      ...
      timer.innerHTML = (days>0?days+'d ':'') + hours + 'h ' + minutes + 'm ' + seconds + 's';
    }, 1000);
  });
}
